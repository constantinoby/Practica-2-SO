...
#define DEBUG5 1                // Debug del ctrlc() y reaper()
...
int execute_line(char *line)
{
    ...
    if (parse_args(args, line)>0)
    {
       ...
       else
       {
        ...
        {
            //asociamos el manejador ctrlc a la señal SIGINT
            signal(SIGINT,ctrlc);
            //Actualizamos datos
            jobs_list[0].pid=pid;
...
void ctrlc(int signum)
{
    ...
    if(jobs_list[0].pid>0)
    {
        ...
        {
            #if DEBUG5
                fprintf(stderr, GRIS_T"[ctrlc()→ Señal %i no enviada por %d (%s) debido a que su proceso en foreground es el shell]\n" RESET, SIGTERM, getpid(),mi_shell);
            #endif
        }  
    }
    ...
    {
        #if DEBUG5
            fprintf(stderr, GRIS_T"[ctlrc()→Señal %i no enviada por %d (%s) debido a que no hay proceso en foreground]\n" RESET, SIGTERM, getpid(),mi_shell);
        #endif
    }
}
