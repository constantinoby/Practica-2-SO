...
#define DEBUG6 0          // Debug del fg y bg
...
int internal_cd(char **args)
{
    ...
    {
        perror(ROJO_T "Error debug internal_cd()" RESET);
    }
...
int internal_bg(char **args)
{
    ...
    if (pos < n_pids && pos >= 1)
    {
        ...
        {
            fprintf(stderr, ROJO_T "bg(%s)→ El trabajo ya está en segundo plano\n" RESET, args[1]);
        }
        else
        {
    ...
    {
        fprintf(stderr, ROJO_T "bg(%s): no existe este trabajo\n" RESET, args[1]);
    }
